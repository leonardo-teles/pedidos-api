...
public class S3Service {
	...
	public URI upload(MultipartFile multipartFile) {
		try {
			...
			return upload(is, nomeArquivo, contentType);
		} catch (IOException e) {
			throw new FileException("Erro de IO: " + e.getMessage());
		}
	}
	
	public URI upload(InputStream is, String nomeArquivo, String contentType) {
		try {
			ObjectMetadata meta = new ObjectMetadata();
			meta.setContentType(contentType);
			
			LOG.info("Iniciando upload");
			s3client.putObject(bucket, nomeArquivo, is, meta);
